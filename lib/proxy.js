...
export function inferCountryCodeFromName(name = "") {
  const value = String(name).trim();
  const leadingCode = value.match(/^([A-Za-z]{2})(?=\s|[-·|])/);
  if (leadingCode) return normalizeCountryCode(leadingCode[1]);
  const symbols = Array.from(value);
  if (symbols.length < 2) return "";
  const first = symbols[0].codePointAt(0);
  const second = symbols[1].codePointAt(0);
